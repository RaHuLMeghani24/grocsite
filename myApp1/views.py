...
def movieList(request):
    # return HttpResponse('This is the about page')
    movie1 = [convert_movieJSONToClass(movie) for movie in fetch_movies(request)]
    return render(request, 'movieList.html', {'movies':movie1})
...
def fetch_movies(request) -> [str]:
    top_movies = list()
    top_movies.append({'name': 'Fight Club', 'url': build_uri(request, '/movies/550'), 'poster_path': 'https://image.tmdb.org/t/p/w500/pB8BM7pdSp6B6Ih7QZ4DrQ3PmJK.jpg'})
